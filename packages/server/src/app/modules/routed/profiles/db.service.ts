...
import { MysqlService, SelectRunOptions } from '@service/mysql/mysql.service';
...
export class DbService {
  ...
  followUser(currentUserId: number, targetUserName: string) {
    return this.mysql
      ...
      .ignore()
      .$run<OkPacket>({}, currentUserId, targetUserName);
  }
  ...
  unfollowUser(currentUserId: number, targetUserName: string) {
    return this.mysql
      ...
      .$run<OkPacket>({}, targetUserName, currentUserId)
      ;
  }
}
